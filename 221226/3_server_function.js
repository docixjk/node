...
const server = http.createServer((req, res) => { //요청 처리 
  ...
  if (myurl.pathname == "/") {
    ...
    let userid = myurl.searchParams.get("userid"); //주소창에서 입력받아서 param userid 뽑아오기;
    res.end(info(userid)); // 끝내면서 함수 
    ...
    let userid = myurl.searchParams.get("userid");//url안에 있는 정보를 읽어 내는거
...
function userReg() {
  ...
  <html lang="ko">
    ...
    <form action="userRegAction">
        <ul>
          <li>
            <label for="userid">아이디</label><input name="userid" type="text" placeholder="userid" />
          </li>
          <li>
            <label for="username">이름</label><input name="username" type="text" />
          </li>
          <li>
            <label for="pw">비밀번호</label><input name="pw" type="password" />
          </li>
          <li>
            <label for="hp">전화번호</label><input name="hp" type="tel" />
          </li>
        </ul>
        <button>추가</button>
      </form>
...
function info(userid) {
  if (!userid || !infoArr[userid]) { //userid 유무 확인후 출력
    return "no user";
  }
  let html = `
  <!DOCTYPE html>
